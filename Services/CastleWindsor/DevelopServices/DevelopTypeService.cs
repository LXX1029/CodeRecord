...
namespace Services.CastleWindsor.DevelopServices
{
    ...
    /// 记录类型数据层
    /// </summary>
    public sealed class DevelopTypeService : Repository, IDevelopTypeService
    {
        /// <summary>
        /// parentId 获取类型对象
        /// </summary>
        ...
        {
            return await this.GetEntities<DevelopType>(m => m.Name == name && m.ParentId == parentId);
        }
        ...
        {
            return await this.GetEntities<DevelopType>(m => m.ParentId == parentId);
        }
        ...
        public override async Task<int> RemoveEntity<T>(T t)
        {
            var affectedRows = 0;
            if (t is DevelopType developType)
            {
                using (var context = new RecordContext())
                using (var trans = context.Database.BeginTransaction())
                {
                    try
                    {
                        var removeRecordList = context.DevelopRecords.Where(x => x.TypeId == developType.Id);
                        if (removeRecordList.Count() > 0)
                        {
                            foreach (var item in removeRecordList)
                                context.Entry(item).State = EntityState.Deleted;
                        }
                        context.Entry(developType).State = EntityState.Deleted;
